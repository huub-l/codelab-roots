...
class CubePortfolioMain {
    ...
    public function populateGoogleFonts($portfolio) {
        ...
        foreach ($fontsArray as $font) {
            ...
            if ($add) {
                ...
                array_push($unique_fonts, $font->slug . ':'. $font->weightStyle);
            }
        }

        if (count($unique_fonts)) {
            $portfolio->style .= '<link rel="stylesheet" href="//fonts.googleapis.com/css?family=' . implode('%7C', $unique_fonts) . '" type="text/css" media="all" property="stylesheet">';
        }
    }
    ...
    public function register_assets() {
        // css
         ...
         wp_register_style('cubeportfolio-jquery-css', CUBEPORTFOLIO_URL . 'public/css/main.min-1.16.3.css', false, CUBEPORTFOLIO_VERSION, 'all');

        // js
        
         wp_register_script('cubeportfolio-jquery-js', CUBEPORTFOLIO_URL . 'public/js/main.min-1.16.3.js', array('jquery'), CUBEPORTFOLIO_VERSION, true);
    }
    ...
    public function register_custom_post_type() {
        ...
        $tax_args = array(
            'hierarchical' => true,
            'label' => __('Custom Categories', CUBEPORTFOLIO_TEXTDOMAIN),
            'singular_label' => __('Custom Category', CUBEPORTFOLIO_TEXTDOMAIN),
            'rewrite' => true,
            'public' => true,
        ...
        if ( get_post_type() == self::$settings['postType'] && is_single()) {
            ...
            if (!$this->request_from_ajax) {
                // init standalone cbp post
                 ...
                 wp_register_script('cubeportfolio-jquery-js-standalone', CUBEPORTFOLIO_URL . 'public/js/init-cbp-standalone.min-1.16.3.js', array('cubeportfolio-jquery-js'), CUBEPORTFOLIO_VERSION, true);
